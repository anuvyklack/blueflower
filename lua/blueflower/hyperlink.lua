...
local get_node_text = _local_3_["get-node-text"]
local scandir_async = require("blueflower.files.scandir")
local _local_4_ = require("blueflower.config")
local os_sep = _local_4_["os-sep"]
local config = _local_4_
local _local_5_ = vim.fn
local getcwd = _local_5_["getcwd"]
local fnamemodify = _local_5_["fnamemodify"]
local _local_6_ = require("blueflower.files.util")
local open_file = _local_6_["open-file"]
local xdg_open = _local_6_["xdg-open"]
local find_and_open_file_async = _local_6_["find-and-open-file-async"]
local _local_7_ = require("blueflower.util")
local notify_error = _local_7_["notify-error"]
local _local_8_ = require("blueflower.files")
local get_current_file = _local_8_["get-current-file"]
...
local function jump_to_link_definition(target)
  ...
  if link_definition then
    local _let_15_ = link_definition
    local link = _let_15_["link"]
    local line_num = _let_15_["line-num"]
    do
