...
Buffer.initialize = function(self, input)
  do
    local _2_ = type(input)
    if (_2_ == "number") then
      self.id = input
    elseif (_2_ == "string") then
      self.id = vim.fn.bufnr(input)
    elseif (_2_ == "nil") then
      self.id = api.nvim_get_current_buf()
    else
    end
  end
  ...
  local function _5_(_, opt, value)
    return api.nvim_buf_set_option(self.id, opt, value)
  end
  self.bo = setmetatable({}, {__index = _4_, __newindex = _5_})
  return nil
end
...
Buffer["get-name"] = function(self)
  local file = api.nvim_buf_get_name(self.id)
  if (file ~= "") then
    return file
  else
    return nil
...
Buffer.delete = function(self, _7_)
  ...
  local unload = _arg_8_["unload"]
  return api.nvim_buf_delete(self.id, {force = (force or false), unload = (unload or false)})
end
Buffer["loaded?"] = function(self)
  return api.nvim_buf_is_loaded(self.id)
end
Buffer["get-var"] = function(self, name)
  return vim.api.nvim_buf_get_var(self.id, name)
end
Buffer["set-var"] = function(self, name, value)
  return vim.api.nvim_buf_set_var(self.id, name, value)
end
...
Buffer["get-text"] = function(self, start_row, start_col, end_row, end_col)
  return api.nvim_buf_get_text(self.id, start_row, start_col, end_row, end_col, {})
end
